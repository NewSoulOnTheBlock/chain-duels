// src/CardPreview.tsx
// Large YGO-style card preview + hover/tap wrapper. Used in-game and in the
// deck builder.
import React, { useState, useRef, useEffect, useCallback } from 'react';
import { createPortal } from 'react-dom';
import { CARDS, COLOR_META, templateFor, isMonster, isSpell, isTrap, type CardDef } from './cards';
...
function statLine(def: CardDef): string {
  if (isMonster(def)) {
    if (def.subtype === 'link') return `LINK-${def.linkRating}  ATK/${def.atk}`;
    if (def.subtype === 'xyz')  return `Rank ${def.rank}  ATK/${def.atk}  DEF/${def.def ?? 0}`;
    return `Level ${def.level ?? 0}  ATK/${def.atk}  DEF/${def.def ?? 0}`;
  }
  if (isSpell(def)) return `[ Spell · ${def.subtype.toUpperCase()} ]`;
  if (isTrap(def))  return `[ Trap · ${def.subtype.toUpperCase()} ]`;
  return '';
}
...
function typeLine(def: CardDef): string {
  ...
  if (isMonster(def)) {
    const tags: string[] = [def.attribute, def.race, def.subtype.toUpperCase()];
    if (def.isTuner) tags.push('Tuner');
    return tags.join(' / ') + ` · ${meta.name}`;
  }
  if (isSpell(def)) return `Spell · ${def.subtype} · ${meta.name}`;
  if (isTrap(def))  return `Trap · ${def.subtype} · ${meta.name}`;
  return meta.name;
}
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', gap: 8 }}>
        ...
        <div style={{ fontSize: 10, fontWeight: 700, opacity: 0.8 }}>{isMonster(def) ? def.attribute : (isSpell(def) ? 'SPELL' : 'TRAP')}</div>
      </div>
      <div style={{
        fontSize: 11, fontWeight: 700, letterSpacing: 1, textTransform: 'uppercase',
        opacity: 0.85, borderBottom: '1px solid #0003', paddingBottom: 4,
      }}>{typeLine(def)}</div>
      <div style={{
        flex: 1, background: 'rgba(255,255,255,0.18)', border: '1px solid #0002',
        borderRadius: 6, padding: 10, fontSize: 13, lineHeight: 1.4, overflowY: 'auto', whiteSpace: 'pre-wrap',
      }}>
        {def.text || (isMonster(def) ? def.flavor ?? '' : '—')}
      </div>
      <div style={{
        alignSelf: 'flex-end', fontWeight: 800, fontSize: 16, lineHeight: 1,
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ fontSize: 10, fontWeight: 800, color: '#1a1a1a' }}>
          {isMonster(def) ? def.attribute : (isSpell(def) ? 'SPELL' : 'TRAP')}
        </div>
      </div>
      ...
      <div style={{
        ...
        fontSize: 10, fontWeight: 700, color: '#1a1a1a', letterSpacing: 0.5,
      ...
      <div style={{
        ...
        padding: '8px 10px',
        fontSize: 11, lineHeight: 1.35, color: '#1a1a1a', overflow: 'hidden',
      }}>
        {def.text || (isMonster(def) ? def.flavor ?? '' : '—')}
        <div style={{
...
/** Hover (desktop) or tap-to-pin (touch) preview wrapper. */
export function CardHover({
  ...
  const onMouseMove = (e: React.MouseEvent) => { if (pos) setPos({ x: e.clientX, y: e.clientY }); };
  ...
  const onTouchStart = (e: React.TouchEvent) => {
    ...
    clear();
    longT.current = window.setTimeout(() => { longFired.current = true; setPos({ x: t.clientX, y: t.clientY }); }, 350);
  };
  ...
  return (
    ...
    >
      {children}
      {pos && def && (
        <FloatingPreview x={pos.x} y={pos.y}><CardPreview def={def} /></FloatingPreview>
      )}
      {pinned && def && (
        <PinnedPreview
          def={def}
...
function PinnedPreview({ def, onClose, onActivate, activateLabel }: {
  def: CardDef; onClose: () => void; onActivate?: () => void; activateLabel: string;
}) {
  return createPortal(
    <div data-cardpreview="1" onClick={onClose} onTouchStart={(e) => e.stopPropagation()}
      style={{
        position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.72)',
        display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
        gap: 14, padding: 16,
      }}>
      <div data-cardpreview="1" onClick={(e) => e.stopPropagation()}><CardPreview def={def} /></div>
      <div data-cardpreview="1" style={{ display: 'flex', gap: 10 }} onClick={(e) => e.stopPropagation()}>
        {onActivate && (
          <button onClick={onActivate} style={{
            background: '#6c4bd8', color: '#fff', border: 'none', borderRadius: 8,
            padding: '12px 22px', fontWeight: 700, fontSize: 16, cursor: 'pointer',
          }}>{activateLabel}</button>
        )}
        <button onClick={onClose} style={{
          background: '#2a2a3e', color: '#fff', border: '1px solid #555', borderRadius: 8,
          padding: '12px 22px', fontWeight: 600, fontSize: 16, cursor: 'pointer',
        }}>Close</button>
      </div>
    </div>,
    document.body,
...
function FloatingPreview({ x, y, children }: { x: number; y: number; children: React.ReactNode }) {
  ...
  return createPortal(
    <div data-cardpreview="1" style={{ position: 'fixed', left, top, zIndex: 9999, pointerEvents: 'none' }}>
      {children}
    </div>,
    document.body,
  );
